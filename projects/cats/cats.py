...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def abt(sentences):
        sentences=lower(remove_punctuation(sentences))
        sl=split(sentences)
        for i in topic:
            for word in sl:
                if i == word:
...
def accuracy(typed, reference):
    ...
    if len(reference_words) ==0 and len(typed_words) == 0:
        ...
        for i in range(0,len(reference_words)):
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    zipped = zip(valid_words, [diff_function(typed_word, i, limit) for i in valid_words])
    if typed_word in valid_words:
        return typed_word
    word = min(zipped, key = lambda x: x[1])
    if word[1] > limit:
        return typed_word
    else:
        return word[0]
...
def sphinx_switches(start, goal, limit):
    ...
    if len(start) == 0 :
        ...
        if limit == 0 :
            return 1       # 当不同字母的数量大于limit时，直接返回 limit+1 ，以减少计算量
...
def pawssible_patches(start, goal, limit):
    ...
    if limit < 0 :  # Fill in the condition
        ...
        add = pawssible_patches(start[0:], goal[1:], limit - 1)  # Fill in these lines
...
def report_progress(typed, prompt, user_id, send):
    ...
    print("ID:",user_id,"Progress:",rate)
    print(rate)
    # END PROBLEM 8
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        time.append([player[i+1]-player[i] for i in range(0,len(player)-1)])
    ...
    for word_index in word_indices:
        ...
        for player_index in player_indices:
            if time(game, player_index, word_index) < min_time:
                ...
                player = player_index
        word_list[player].append(word_at(game, word_index))
    return word_list
    # END PROBLEM 10
