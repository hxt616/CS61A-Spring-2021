...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return 3+int(min(list(str(score**2))))
    # END PROBLEM 2
...
def more_boar(player_score, opponent_score):
    ...
    def num_list(n):
        if n//10 == 0:
            ...
            p=list(str(n))
        return p
    player=num_list(player_score)
    opponent=num_list(opponent_score)
    if player[0]<opponent[0] and player[1]<opponent[1]:
        return True
    else:
        return False
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        ...
        if gain>running_high:
            ...
            return announce_highest(who,last_score+gain,gain)
        return announce_highest(who,last_score+gain,running_high)
    return say 
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    sum=[0]
    st = make_averaged(roll_dice,trials_count)
    for i in range(1,11):
        sum.append(st(i,dice))
    for j in range(1,11):
        if sum[j]==max(sum):
            return j
    # for i in range(1,11):
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if more_boar(score,opponent_score):
        return 0
    num_rolls = piggypoints_strategy(score, opponent_score, cutoff, num_rolls)
    return num_rolls
    #return 6  # Replace this statement
    # END PROBLEM 11
