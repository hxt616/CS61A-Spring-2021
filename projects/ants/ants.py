...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        "*** YOUR CODE HERE ***"
        if exit is not None:
            # self.exit表示当前位置的左边的一格，而self.exit.entrance表示exit位置右边的一格，即为self本身所处的位置
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not now_place.is_hive() :
            if now_place.bees and self.min_range<=count<=self.max_range:
                return bee_selector(now_place.bees) 
            now_place = now_place.entrance  # 向右移动一格
            count+=1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees and self.chewing==0:
            the_bee = bee_selector(self.place.bees)
            the_bee.health = 0
            the_bee.reduce_health(0)  # 移除该蜜蜂
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        if other.is_container() or self.contained_ant!=None:
            return False
        else:
            return True
        # END Problem 8
    ...
    def action(self, gamestate):
        ...
        if self.contained_ant:
            self.contained_ant.action(gamestate)  # 当容器内部有蚂蚁时，调用容器内蚂蚁的action
        # END Problem 8
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if not self.flag:
            ...
            Ant.action(self,gamestate)
            behind_place = self.place.exit  #蚁后的后面位置是当前位置的exit处
            while behind_place:
                if behind_place.ant and not behind_place.ant.is_container():
                    behind_place.ant.buff()
                elif behind_place.ant and behind_place.ant.is_container():
                    behind_place.ant.buff()
                    if behind_place.ant.contained_ant:
                        behind_place.ant.contained_ant.buff()
                behind_place = behind_place.exit
            self.throw_at(self.nearest_bee(gamestate.beehive))   # 蚁后自身也会攻击蜜蜂
        # END Problem EC
