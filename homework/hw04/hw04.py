...
def totals_tree(m):
    ...
    if is_planet(m):
        return [size(m)]
    labels = total_weight(m)
    branches = [totals_tree(i) for i in [end(left(m)),end(right(m))]]
    return tree(labels,branches)    
...
def replace_thor_at_leaf(t, thors_replacement):
    ...
    branch = [replace_thor_at_leaf(i, thors_replacement) for i in branches(t)]
...
def has_path(t, word):
    ...
    return label(t) == word[0] and any([has_path(i, word[1:]) for i in branches(t)])
