...
def subseqs(s):
    ...
    if s == []:
        ...
        subset = subseqs(s[1:])     # pythontutor的图看起来有点乱，建议手写过程理解代码
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    "*** YOUR CODE HERE ***"
    # 计算包含n个叶子的完全二叉树的个数
    # 参考https://math.stackexchange.com/questions/4128699/t-n-a-set-of-all-binary-trees-with-n-leaves
    if n==1 or n==2:
        return 1
    else:
...
def merge(incr_a, incr_b):
    ...
    while next_a is not None and next_b is not None:
        if next_a < next_b:
            ...
            yield next_a
            next_a, next_b = next(iter_a, None), next(iter_b, None)
    while next_a is not None:  # next_b为None
        yield next_a
        next_a = next(iter_a, None)
    while next_b is not None:  # next_a为None
        yield next_b
        next_b = next(iter_b, None)
...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        result = ''
        for ty in typing_input:
            self.buttons[ty].times_pressed+=1
            result+=self.buttons[ty].key
        return result
...
class Account:
    ...
    def __repr__(self):
        "*** YOUR CODE HERE ***"
        return f"Accountholder: {self.holder}, Deposits: {self.deposit_num}, Withdraws: {self.withdraw_num}"
...
def trade(first, second):
    ...
    while m <= len(first) and n <= len(second) and not equal_prefix():
        if sum(first[0:m]) < sum(second[0:n]) and m<len(first):
            m += 1
        else :
            n += 1
...
def deep_len(lnk):
    ...
    if not lnk:
        return 0
    elif isinstance(lnk, int): # 判断当前传入的元素是否是整型
        return 1
    else:
...
def prune_small(t, n):
    ...
    while len(t.branches)>n:
        largest = max(t.branches, key=lambda x: x.label)  # max(iterable, *iterables, key, default)
...
def long_paths(t, n):
    ...
    "*** YOUR CODE HERE ***"
    tree_list = []
    if t.is_leaf() and n<=0:  # n<0表示列表中的元素大于n个
        tree_list.append([t.label])
    for branch in t.branches:
        for path in long_paths(branch, n-1):
            tree_list.append([t.label]+path)
    return tree_list
